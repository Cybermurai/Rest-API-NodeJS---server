...
mongoose.connect('mongodb://localhost:27017/Zguba', {useNewUrlParser: true, useUnifiedTopology: true});
const connection = mongoose.connection;
const cors = require('cors');

console.log('--------------------------------'.red);
console.log('Welcome to '.green+'Zguba'.red+' server'.green);
...
app.get("/api/users/:id?", (req, res)=>{
    connection.db.collection("Accounts", function(err, UsersCollection){
          if(req.query.id){
...
app.get("/api/auth/:email?/:psw?", (req, res)=>{
  if(!req.query.email|| !req.query.psw){
    return res.json({error:true, auth:false, msg: "Empty login or password value"});
  }else{
    connection.db.collection("Accounts", (err, UsersCollection)=>{
        UsersCollection.find({email:req.query.email, psw:req.query.psw}).toArray((err,data)=>{
          if(err){
            return res.json({error:true, auth:false, msg: err});
          }else{
            if(!data[0]){
              return res.json({error:true, auth:false, msg:"Incorrect email or password"});
            }else{
...
/*Pobranie danych ITEMU/BRELOCZKA po uid*/
app.get("/api/item/:uid?", (req, res)=>{
  connection.db.collection("Items", function(err, ItemsCollection){
        if(req.query.uid){
          let uid = req.query.uid;
          ItemsCollection.find({uid:uid}).toArray(function(err, data){
            if(err) console.log("err");
            return res.json(data); 
          }) 
        }
